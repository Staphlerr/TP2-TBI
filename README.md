...
* **Automated Multi-Model Evaluation:** Integrated and evaluated search effectiveness using industry-standard metrics across TF-IDF, BM25, and WAND concurrently:
...
* `index/`: Directory where the final compressed inverted index binaries and metadata are stored (tracked via `.gitkeep`).
...
* `search.py`: A script to perform interactive Top-K searches comparing TF-IDF, BM25, and WAND simultaneously.
...
To test the system's accuracy against the provided 30 queries (`queries.txt`) and view the comparative evaluation scores (RBP, DCG, NDCG, AP) for TF-IDF, BM25, and WAND, run:
```bash
python evaluation.py
```

### 3. Running Interactive Searches
You can modify the sample query list inside `search.py` and run the script to see the Top-5 retrieved documents along with their scores across all three retrieval methods:
```bash
python search.py
```
